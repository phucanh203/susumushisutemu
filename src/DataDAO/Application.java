...
public class Application {
	...
	public void setNameIDByUrl(String url)
	{
		...
		String id = arr[arr.length-1].substring(0,arr[arr.length-1].length() - 5);
		setAppID(id);
	}
	...
	public void setAppDetailByElement(Element elment)
	{
		...
		for(Element detailApp : detailApps)
		{
			...
			case "Version":
				...
				System.out.println("Version: "+detailApp.text().substring(7));
				break;
			case "Size" :
				setSize(detailApp.text().substring(4));
				System.out.println("Size: "+detailApp.text().substring(4));
				break;
			case "Updated":
				//setUpdatedDate((detailApp.text().substring(7));
				//System.out.println("Updated: "+detailApp.text().substring(7));
				break;
			
			}
			//System.out.println(detailApp.html().toString());	
		}
	}
}
